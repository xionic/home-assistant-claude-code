...
> Auto-generated by Claude Code UI app. Run \`ha-context\` to refresh.
...
APIREF
    ...
    # Other apps' config folders — only advertised when the option enables it.
...
## Other Apps' Configuration — `/addon_configs`

Access to other apps' config folders is **enabled**. Each app's config is
mounted at `/addon_configs/<repo>_<slug>/` — e.g. `/addon_configs/local_myaddon/`
or `/addon_configs/a0d7b954_zwavejs/` (`local_` for locally-installed apps, a
short hash for store apps). Read and edit these to help configure other
apps. Treat them as carefully as `/config`: they often hold secrets, and the
owning app usually needs a restart to pick up changes — confirm with the user
before editing.
ADDONCFG
    fi
...
The \`esphome\` CLI is available. Device YAML lives in \`${ESPHOME_CONFIG_DIR:-the ESPHome app config folder}\`
